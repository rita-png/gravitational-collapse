...
function RHS(y0,x1,time,func,i,data)
    ...
    derxxchi = func[1] #this is (xxchi),r in non compactified code but is (xxchi),x in compactified
    derpsi = func[2]
    xxchi = func[3]
    derrxxchi = func[4]
...
function SF_RHS(data,t,X)
    ...
    derxxchi_func = Spline1D(X[4:L-3], data[4:L-3,5],  k=4);
    derpsi_func = Spline1D(X[4:L-3], data[4:L-3,6],  k=4);
    xxchi_func = Spline1D(X[4:L-3], data[4:L-3,7],  k=4);
    derrxxchi_func = Spline1D(X[4:L-3], aux[4:L-3], k=4);
...
function timeevolution(state_array,finaltime,run)#(state_array,finaltime,dir,run)
    ...
    while t<finaltime#@TRACK
        ...
        # update interpolation of psi,x
        derxxchi_func = Spline1D(X[4:L-3], state_array[4:L-3,5],  k=4);
