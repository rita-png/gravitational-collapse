...
function derxi_evol(y,i,X)
    ...
    if compactified == false
        ...
        x=X[i]
        dy=((1/(2*x^2))*exp(2*y[i,2])*(-1+x)^2*(1+y[i,3]-(1+y[i,3])^3+2*(-1+x)^2*y[i,4]*(x*((-1+x)*Der(y,i,1,X)+x*Der(y,i,2,initX))+y[i,1]*(1+2*(-1+x)*x*Der(y,i,2,initX)))+(x*(x+2*(-1+x)*y[i,1])*Der(y,i,4,initX))/(-1+x)^2))

        #shittdy=1/((-1+x)^2) * ((1/(2*x^2))*exp(2*y[i,2])*(-1+x)^2*(1+y[i,3]-(1+y[i,3])^3+2*(-1+x)^2*y[i,4]*(x*((-1+x)*Der(y,i,1,X)+x*Der(y,i,2,initX))+y[i,1]*(1+2*(-1+x)*x*Der(y,i,2,initX)))+(x*(x+2*(-1+x)*y[i,1])*( Der(y,i,4,initX)* (-1+x)^4 + Der(y,i,4,initX)*2*(-1+x)^3 ))/(-1+x)^2))
...
function RHS(y0,x1,time,func,i,data)
    ...
    if compactified==true
        if abs.(x1)<10^(-15)
            ...
            x=x1
            z[1]=(2*pi*(y0[3]^2*(2+y0[3])^2+2*(-1+x)^2*x*(x+2*(-1+x)*y0[1])*(derxi(x1))^2))/x^2
        end
    else
        z[1]=0#?
    end
